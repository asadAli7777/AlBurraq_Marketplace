...
export default function NewItem() {
  ...
  const nft=useSelector((state) =>(state.nft.value))
    ...
    // const [registration,setRegistration] = (false);
    ...
    const buyLazyMarketItem = async (item) => {
      ...
      alert(`Your Token Id Is ${Number(tokenIDD)}`)
    }
    ...
    const createNFT = async (e) => {
      ...
      if (!image || !price || !name || !description || !account || amount === 0 || !rarity) {
        ...
        try {
          ...
          // alert("ipfs uri upload error: check your internet connection")
          console.log(error)
        }
      }
    }
    ...
    const mintThenList = async (result) => {
      ...
      try {
       ...
       setTransactionState(true)
        ...
        alert("Congratulations you have Listed your Nft successfully")
      } catch (error) {
        setTransactionState(false)
        alert(error.data);
      }
    ...
    const listItem = async (e) => {
      ...
      try {
        await (await nft.setApprovalForAll(marketplace.address, true)).wait()
        // add nft to marketplace
        const _price = listingPrice.toString();
        const listingPriceInWei = ethers.utils.parseEther(_price);
        await (await marketplace.makeItem(nft.address, listingId, listingPriceInWei, listingAmount)).wait()
